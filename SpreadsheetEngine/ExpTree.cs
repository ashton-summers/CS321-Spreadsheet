...
using System.Linq;
using System.Text;
using System.Threading.Tasks;/*    * Name: Ashton Summers    * Date: 9/30/16    * ID: 11414124    * Assignment Description: Creates an expression tree based on user inputs. For this assignment, users will not enter an    *              expression with multiple operators. Once the tree is built, the user has the choice to evaluate     *              its result */
namespace CptS321
{
    public class ExpTree
    {
        private Dictionary<string, double> mVariables = new Dictionary<string, double>(); private Node mRoot;
        ...
        //Sets data for specified varaible in a dictionarys
        ...
        public Node BuildTree(ref string exp)
        {
            ...
            if (temp != null)
            {
                exp = exp.Substring(index + 1); //finds everything to right of operator and determines if it a variable or number           
        ...
        public double Eval()
        {
            double result = CalculateValue(mRoot); return result;
        }
